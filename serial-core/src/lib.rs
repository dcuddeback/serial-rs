...
/// The `GetReadableBytesCount` trait allows for getting the number of bytes
/// received by the serial device.
pub trait GetReadableBytesCount {
    /// Returns the number of readable bytes for the serial port.
    ///
    /// This function is useful to read unknown number of bytes without timeout.
    ///
    /// ## Errors
    ///
    /// This function returns an `Io` error for any type of I/O error.
    ///
    /// ## Example
    ///
    /// ```no_run
...
pub trait SerialDevice: io::Read + io::Write + GetReadableBytesCount {
...
pub trait SerialPort: io::Read + io::Write + GetReadableBytesCount {
    /// Returns the current timeout.
    fn timeout(&self) -> Duration;
